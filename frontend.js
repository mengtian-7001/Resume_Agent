  ...
  try {
    ...
    const candidates = matches.map((match) => {
      ...
      const drawerProfile = {
        ...
        scores: scoreEntries,
        semanticComparison: {
          semantic: Number.isFinite(Number(breakdown.text_semantic)) ? Number(breakdown.text_semantic) : null,
          tfidf: Number.isFinite(Number(breakdown.text_tfidf)) ? Number(breakdown.text_tfidf) : null,
          hybrid: Number.isFinite(Number(breakdown.text ?? breakdown.text_score)) ? Number(breakdown.text ?? breakdown.text_score) : null,
          source: String(breakdown.text_semantic_source || "unknown"),
        },
      ...
      return {
        ...
        risks: risks.map((item) => (typeof item === "string" ? item : item?.text || "")).filter(Boolean),
        scores: scoreEntries,
        semanticComparison: drawerProfile.semanticComparison,
        gates: checker.gates,
        questions,
        followups,
      };
