...
一线招聘的筛选，很少是「一份简历、一个明确结论」。更常见的是：一个岗位在几天内收到几十到上百份材料，格式从精美 PDF、扫描件到纯文本都有；招聘人员往往只有几十秒到两分钟决定要不要推进，用人经理随后会问「为什么是这几个人，依据在哪」。

现有做法容易卡在两头。纯靠人读，标准会随班次、疲劳和紧急程度漂移，同一份简历不同同事可能给出不同结论，事后也难复盘。纯靠关键词或传统 ATS，又会漏掉同义表述（例如简历写「PySpark / 气流调度」，JD 写「Spark / Airflow」），同时放行把名词堆满、项目却写不清的简历。进入面试后，面试官还常常要从零准备问题，大量时间耗在确认「了解、参与、预研」到底有没有做过。

简历中台按这个真实筛选场景来做：系统把非结构化的 JD 和简历变成可比较、可追溯的优先级，标出硬门槛、原文证据和待验证风险；录用与否仍由招聘人员和用人经理决定。它不是自动录用系统，而是把初筛从「凭印象翻页」变成「带着证据开会」。

## 真实筛选中的痛点

| 痛点 | 招聘现场实际发生什么 | 如果不管会怎样 |
| --- | --- | --- |
| 材料量大、格式不齐 | 一次筛选混着 PDF、DOC、DOCX、扫描件；JD 有时是文件，有时是粘贴文本 | 时间先耗在打开文档和找字段上，而不是判断适不适合 |
| 判断标准随人漂移 | 没有统一尺子时，推荐名单依赖个人经验和当天状态 | 用人经理只看到「感觉不错」，追问依据时对不上原文 |
| 关键词既误伤又误放 | 同义词、别名、中英混写会挡住合格人；技能词堆砌或粘贴 JD 的空简历却容易过关 | 漏掉能干活的人，把包装好的人推进面试 |
| 表述膨胀，缺少证据 | 「了解 / 参与 / 预研 / Demo」被写成「精通 / 主导 / 生产级」 | 初筛无法区分真实交付和自我包装 |
| 硬门槛被「看起来很像」带过 | 年限、学历、必备技能本应一票否决，跨领域资深头衔或概括性描述容易让人忽略缺口 | 灰区候选人被直接放行或直接丢掉，两边都贵 |
| 过了初筛，面试仍从零开始 | 面试官缺少针对该候选人缺口和风险的问题 | 现场时间花在核实简历有没有写清楚，而不是验证能力和团队匹配 |
| 简历是敏感个人信息 | 含联系方式、薪资预期、教育与任职经历 | 不能进共享盘或未隔离的处理链路 |
| 系统不能代替录用决定 | 筛错人有业务成本和用人风险 | 产品只给优先级和证据，最终面试、背调和录用仍由人完成 |
...
- 新克隆直接运行 `./dev.sh`：本地页面自动接入公开匿名 Supabase 与线上 Worker，真实执行上传、解析、评分、出题和 Checker。
- 配置自有 Supabase 后，`./dev.sh --local` 自动安装 Python 依赖并启动本地 Worker；`./dev.sh --demo` 才是纯静态备用模式。
- 演示路径见 [docs/DEMO.md](docs/DEMO.md)，部署说明见 [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)，评测案例见 [docs/EVALUATION_CASES.md](docs/EVALUATION_CASES.md)。
...
| 面试准备材料 | 每位候选人至少 10 题，高分候选人生成 3–5 个追问 | Construction Agent |
| 独立质检 | 检查引用、归因、能力强度、硬门槛、分数和题目结构 | `agents.py` |
| 校准与修复建议 | 输出问题类型、严重度、建议动作和可执行 patch | `checker_corrections.py` |
| 有界回修 | 未通过时最多回修一轮；结论只能变得更保守 | `checker_harness.py` |
| 可观测与可复现 | 记录 Plan / Act+Observe / Reflect、降级和修订轨迹 | `agent_trace.py` |

阅读顺序建议：先看「开发背景 → 架构与数据流 → 核心难点与解决方案 → Agent 工作流 → Prompt 设计思路」，需要落地细节时再看数据库、向量、Harness、测试和部署章节。
...
这一节写的是把真实筛选场景做成稳定系统时，真正难卡住的地方。前面的痛点回答“为什么做”，这里回答“为什么难做稳、怎样验证已经稳住”。
...
核心取舍是：**让 LLM 处理语义，让规则守住门槛，让 Checker 找错，让 Harness 保证系统能停、能追溯、能交付。**

### 痛点如何落到实现

| 真实痛点 | 系统怎么处理 |
| --- | --- |
| 材料量大、格式不齐 | 解析 PDF / DOC / DOCX，扫描件自动 OCR；JD 支持上传或直接粘贴；按工作区隔离一批筛选 |
| 判断标准随人漂移 | 硬门槛冻结后不可改写；混合评分给出可解释分项；`agent_runs` 留下逐步轨迹，开会时能对上原文 |
| 关键词既误伤又误放 | 技能同义归一后再覆盖；关键词堆砌记为风险，不能单独换成「生产级」结论 |
| 表述膨胀，缺少证据 | 能力只能引用 JD / 简历原句；「了解 / 参与 / Demo」不能升级成「精通 / 主导」；Checker 独立挑刺 |
| 硬门槛被「看起来很像」带过 | 年限、学历、必备技能由规则守门，LLM 不能推翻；灰区进入人工复核，结论只允许变严 |
| 过了初筛，面试仍从零开始 | 按候选人证据和缺口生成结构化问题与追问，让面试官带着待验证点进场 |
| 简历是敏感个人信息 | 私有 Storage、按 `workspace_id` 的 RLS、浏览器只持有 anon key，服务端密钥不进前端 |
| 系统不能代替录用决定 | 输出是优先级、证据和风险，不是录用指令；招聘人员和用人经理做最终判断 |
...
脚本会自动读取公开的浏览器安全配置，本地启动前端，并连接 [resume.flowsome.top](https://resume.flowsome.top) 的匿名 Supabase 与线上 Worker。选择样例 JD 和简历后点击「一键解析」，会真实执行文档上传、结构化解析、混合评分、面试题生成、Checker 质检、结果落库和候选人详情展示。

这里“不需要安装后端依赖”是因为默认模式复用公开云端 Worker，**不是静态假数据**。每位访客会获得独立匿名工作区；敏感或正式业务简历建议使用下方的自托管模式。
...
端口被占用时可指定其他端口：`PORT=4180 ./dev.sh`。服务器默认只绑定 `127.0.0.1`，按 `Ctrl+C` 停止。退出时脚本会删除临时下载的公开配置。
...
脚本会自动创建 Python 虚拟环境、安装锁定依赖并启动全部服务。以后直接运行 `./dev.sh`，检测到有效的自有配置后也会自动进入本地 Worker 模式。`--live` 保留为 `--local` 的兼容别名。
