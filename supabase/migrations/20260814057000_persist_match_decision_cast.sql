-- jsonb ->> returns text; match_results.decision is match_decision.
-- Cast explicitly so persist_screening_candidate_core does not fail with 42804.
...
declare
  ...
  mapped_decision public.match_decision;
...
begin
  ...
  mapped_decision := case lower(coalesce(p_match->>'decision', 'review'))
    when 'recommend' then 'recommend'::public.match_decision
    when 'reject' then 'reject'::public.match_decision
    else 'review'::public.match_decision
  end;
